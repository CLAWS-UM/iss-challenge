...
public class MissionPanel : MonoBehaviour {
    ...
    public class Mission {
    	...
    	public struct Mission_Task : Mission_Subtask {
        ...
        public struct Mission_Phase : Mission_Subtask {
            Mission_Task[] tasks;
            int tasks_complete;
            // TO DO: Insert Navigational Component
        ...
        public Mission() {
            ...
            for(int i = 0; i < 5; i++){
                s[i] = new Mission_Subtask("Subtask " + i, "Fix something");
                total_subtasks++;
            }
            Mission_Task[] t = new Mission_Task[5];
            for(int i = 0; i < 5; i++){
                t[i] = new Mission_Task("Task " + i, "Fix something", s);
            }
            phases = new Mission_Phase[5];
            for(int i = 0; i < 5; i++){
            	phases[i] = new Mission_Phase("Phase " + i, "Fix something", t);
            }
        }
        ...
        public Mission(TextAsset missionFile) {
            ...
            // Populate task array
            int numPhases = lines[3];
            phases = new Mission_Phase[numPhases];
            int it = 4;
            for(int i = 0; i < phases.Length; i++){
            	phases[i] = new Mission_Phase(lines[it], lines[it+1]);
            	it += 2;
                
                int numTasks = lines[it];
                phases[i].tasks = new Mission_Task[numTasks];
                for(int k = 0; k < phases[i].tasks.Length; k++){
                    phases[i].tasks[k] = new Mission_Task(lines[it], lines[it+1]);
                    it += 2;

                    int numSubtasks = lines[it];
                    phases[i].tasks = new Mission_Subtask[numSubtasks];
                    for(int j = 0; j < phases[i].tasks[k].subtasks.Length; j++){
                        phases[i].tasks[k].subtasks[j] = new Mission_Subtask(lines[it], lines[it+1]);
                        it += 2;
                        total_subtasks++;
                    }
                }
        ...
        public ref Mission_Task get_cur_subtask(){
            return phases[phases_complete].get_cur_task().get_cur_subtask();
        }
        public ref Mission_Task get_prev_subtask(){
            if(phases[phases_complete].get_cur_task().subtasks_complete > 0)
                return phases[phases_complete].get_cur_task().get_prev_subtask();
            else if(phases[phases_complete].tasks_complete > 0)
        ...
        public ref Mission_Task get_next_subtask(){
            if(phases[phases_complete].get_cur_task().subtasks_complete < 
                    phases[phases_complete].get_cur_task().subtasks.Length)
                return phases[phases_complete].get_cur_task().get_next_subtask();
            else if(phases[phases_complete].tasks_complete < phases[phases_complete].tasks.Length)
        ...
        public void toggle_status(){
            Mission_Subtask stog = get_cur_subtask();
            stog.toggle();
            if(stog.get_status()){
                num_subtasks_complete++;
            }
            int old_phase = phases_complete;
            if(phases[phases_complete].get_cur_task().subtasks_complete == 
                    phases[phases_complete].get_cur_task().subtasks.Length){
                phases[phases_complete].get_cur_task().toggle();
            }
            if(phases[phases_complete].tasks_complete == phases[phases_complete].tasks.Length){
                phases[phases_complete].toggle();
            }

        	// If phase marked completed, increment
        	if(phases_complete > old_phase){
        		phases_complete++;
        	}
        	// Else decrement
        	else if(phases_complete < old_phase) {
        		phases_complete--;
        	}
        	// Update progress
